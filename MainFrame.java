...
//Главное окно ИС "Автопарк"
    ...
    static String helpArr1 =
            ...
            "     Галаев Олег Олегович\n" +
            "     СевГУ - 2025.\n";

    static String helpArr2 =
        "\n     Информационная система для хранения и обработки\n" +
        "     данных об автомобилях (ID, марка, цена, год).\n" +
        "\n     Основные функции:\n" +
        "     • загрузка и сохранение базы;\n" +
        "     • редактирование записей;\n" +
        "     • сортировка и фильтрация;\n" +
        "     • выборки по цене и марке;\n" +
        "     • итоги по годам.\n" +
        "\n     Результаты отображаются в таблице.\n";

    public MainFrame() {
        // 1. Внутреннее представление таблицы
        Global.table = new CarFleet("Список записей об автомобилях");

        // 2. Список для JTable
        Global.cars = new ArrayList<Car>();

        // 3. Модель и JTable
        Global.tableModel = new CarTableModel(Global.cars);
        ...
        int WinSizeG = 700;
        int WinSizeV = 500;
        frame = new JFrame("Система хранения и обработки данных об автомобилях");
        ...
        myC.add(new JLabel("Информация об автомобилях", JLabel.CENTER),
                BorderLayout.NORTH);

        MSG = new JLabel("   Информационная система по автомобилям");
    ...
    public void OpenFile() {
        ...
        if (rez == JFileChooser.APPROVE_OPTION) {
            ...
            java.util.List<Car> carList = Transfer.StringsToCars(LINES);
            if (carList != null) {
                Global.table.getCars().clear();
                for (Car car : carList) Global.table.addCar(car);
    ...
    public void SaveFile(boolean fs) {
        ...
        if ((!curFile.exists()) || fileName.equals(old_file_name)) {
            LINES = Transfer.CarsToStrings(Global.table.getCars());
            try {
